...
public class ChessMatch {
    ...
    public ChessPiece performChessMove(ChessPosition sourcePosition, ChessPosition targetPosition){
        ...
        // specialMove en passant - Testar se a peca modida foi um peao e movel duas casas

        if (movedPiece instanceof  Pawn && (target.getRow() == source.getRow() - 2 || target.getRow() == source.getRow() + 2)){
            enPasantVulnearble = movedPiece;
        }else {
            enPasantVulnearble = null;
        }
    ...
    private Piece makeMove(Position source, Position target){
        ...
        if (p instanceof Pawn ){
            // source.getColumn() != target.getColumn()  -> verifica se andou na diagnonal
            // verificar se foi um en passan
    ...
    private void undoMove(Position source, Position target, Piece capturePiece){
        ...
        if (p instanceof Pawn ){
            // source.getColumn() != target.getColumn()  -> verifica se andou na diagnonal
            // verificar se a peca captura fou um en passan
            if (source.getColumn() != target.getColumn() && capturePiece == enPasantVulnearble) {
